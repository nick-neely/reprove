...
**It names `@reprove/worker-hosted`, and that declaration is what makes this deployment hosted.** `@reprove/control-plane-workflow` carries the hosted driver as an **optional peer** and imports it lazily ([#57](https://github.com/nick-neely/reprove/issues/57)), so nothing installs it on that package's behalf: pnpm auto-installs only non-optional peers, and the composition root is what decides. This app is the hosted root, so it declares it; a self-hosted root declares neither it nor anything requiring it, and installs no harness code at all. `tools/verify-workspace.mjs` asserts both halves over the whole package graph - that this app declares the driver, and that the only path from it to `@reprove/worker-core` runs through that driver, so removing the driver removes the harness stack. It imports `@reprove/control-plane-workflow/hosted` for the same reason it declares the driver: that subpath is the half of the orchestration package whose declarations name the driver, and importing it is what puts the hosted pass in the module graph the Workflow build registers workflows from.
