...
`dispatchHostedPass` takes an Owner and a Run and nothing else. [ADR 0016](../../docs/adr/0016-phase-0-acceptance-scenario.md)'s abandoned case - a pass genuinely running against a Run that records none, which is why execution liveness covers the whole of Acceptance's eligibility window rather than `executing` alone - is reached in `spine.test.ts` through `HostedDispatchPorts` instead: the claim and the pass are the real ones, and only the `markExecuting` port is replaced by one that never returns. That leaves the same row the window is defined by. [#87](https://github.com/nick-neely/reprove/issues/87) removed the `HostedDispatchOptions` parameter this used to forward for that case, along with the test-only branch on the far side of it.
