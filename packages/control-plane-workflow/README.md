...
**The loser is whoever the row does not name, and that takes a read.** A `record` that matches nothing used to be read as "somebody else won", which held while this step was the column's only writer. It is not: a lifecycle records itself once its deadline and the record grace have both passed with the column empty ([#85](https://github.com/nick-neely/reprove/issues/85)), and cancelling on the failed write alone would then kill the lifecycle the Run records - leaving the Run naming a cancelled durable run, both windows open, and every platform retry repeating the cancellation. So the step reads the Run back and compares: its own id is the winner and is kept, another id is the orphan and is cancelled, and a Run this Owner cannot see at all is cancelled too, because a durable run nobody will ever record must not be left sleeping. The read is on that path only, so an ordinary dispatch is the two round trips it always was. `concludeDispatch` is the comparison, exported beside the step for [`ingress.test.ts`](src/ingress.test.ts) and not from the entry point, because the step's own path needs an interleaving no test can produce without a test-only branch in shipped orchestration.
...
Every lifecycle-side transition is conditional on the writer being the lifecycle the Run records, so an orphan wakes, matches nothing, and ends.

**A deadline that passes while nothing is recorded is waited out and then recorded, rather than treated as orphanhood.** `dispatchLifecycle` starts before it records, so a Run whose lifecycle has not yet been written has two possible causes the loop cannot tell apart from inside: the dispatching step crashed in that window, or the write is simply still in flight. The loop waits two seconds per wake for five wakes, which separates them without asking - a write that is coming lands inside that, and one that never comes has been given every chance to.

Then the lifecycle **records itself**, whichever it was, because nothing else can ([#85](https://github.com/nick-neely/reprove/issues/85)). Every lifecycle-side transition carries `workflow_run_id = <the writer>`, so a Run past a deadline that names no lifecycle has no writer either window will accept: it would stay `queued` past `claimableUntil`, or `claimed` and Result-eligible, until something that is not a deadline ended it - the prompt detector's execution token, or a supersession - and neither is guaranteed to arrive. Returning instead left exactly that, because the predicate the return was sparing would have matched nothing anyway. The write is the control plane's own `IS NULL`-guarded statement, so the row still arbitrates and a lifecycle that loses it reads somebody else's id on the next wake and ends as the orphan it turned out to be. Waiting first is what keeps the ordinary case ordinary, and ten seconds is many times one database round trip and a small fraction of the shorter of the two Phase 0 windows, the five-minute claim window; the grace covers both. `claimableUntil` bounds the unclaimed window and nothing else: it writes exactly one transition, `unscheduled`, over a Run that was never claimed.
...
**Which is also where a hosted pass's structured Failure ends up, today.** Phase 0 has no transition for a Failure or a Refusal from Worker core, so `@reprove/worker-hosted` returns either as the pass's own value and writes nothing; the durable run ends `completed`, and this branch closes the Run `failed(worker_lost)` with `workflow_terminal_without_result`, keeping none of the reason, phase or detail. Reading the pass's return value instead would not fix it - there would still be no failure reason to write, since `RUN_FAILURE_REASONS` has the single member `worker_lost` - so it is recorded as a gap rather than patched here, and received by [#83](https://github.com/nick-neely/reprove/issues/83). It is unreachable in the shipped Phase 0 composition, whose Worker core produces a fixture Result or throws, and `spine.test.ts` drives a pass that ends in a structured Failure to its deadline and pins exactly this outcome.
...
**`@reprove/worker-hosted` is an optional peer** - `peerDependencies` plus `peerDependenciesMeta.optional` - imported lazily by `hostedPlacement()`, and named in `devDependencies` besides so this package can type-check and test against it. That is [ADR 0010](../../docs/adr/0010-package-graph-and-open-core-boundary.md)'s deployment table as an edge: a hosted deployment composes it, a self-hosted one omits it, and *"a control plane that dispatches only to self-hosted Workers installs no harness code at all"* is only true if this package runs without it. So its absence is an answer rather than a crash - `null` composes no hosted dispatch, every step above answers `not_composed`, and the webhook, the claim endpoint, Acceptance and the lifecycle are untouched. A package that is present and *broken* is rethrown instead, because answering `null` there would report a defective deployment as a self-hosted one; only a resolution failure that names this driver itself reads as absence, so a driver whose own dependency is missing is reported rather than mistaken for a self-hosted deployment.
...
### How the abandoned case ADR 0016 needs is reached

`dispatchHostedPass` takes an Owner and a Run and nothing else. [ADR 0016](../../docs/adr/0016-phase-0-acceptance-scenario.md)'s abandoned case - a pass genuinely running against a Run that records none, which is why execution liveness covers the whole of Acceptance's eligibility window rather than `executing` alone - is reached in `spine.test.ts` through `HostedDispatchPorts` instead: the claim and the pass are the real ones, and only the `markExecuting` port is replaced by one that rejects before its durable write lands. That leaves the same row the window is defined by. [#87](https://github.com/nick-neely/reprove/issues/87) removed the `HostedDispatchOptions` parameter this used to forward for that case, along with the test-only branch on the far side of it.
...
The tests run this package's workflows under a real Workflow builder: `@workflow/vitest` compiles every `'use workflow'` and `'use step'` function it discovers into bundles and executes them in-process against the local World. `spine.test.ts` is therefore measuring what [ADR 0014](../../docs/adr/0014-workflow-orchestration-seam.md) could only decide on paper - that a created Run reaches a claimable state through the real workflow runtime rather than a stub, that the `run` row arbitrates between two lifecycles, that the unclaimed window closes as `unscheduled` and nothing else, that a claimed Run nobody came back for ends `failed(worker_lost)` from the same durable run, that a lifecycle the row names nobody for records itself after the grace and closes whichever of the two windows it was watching, that a hosted Run reaches Worker core and its Result reaches Acceptance, that the watchdog terminalizes before it cancels the pass and names what that pass did, and that the re-drive of a contended delivery is the platform's own step retry - with GitHub substituted at the transport and nowhere else.

The watchdog cases start a **stand-in** durable run rather than a hosted pass, and `pass.test-support.ts` says why: the shipped pass composes the Phase 0 fixture Worker core, so it submits a Result within milliseconds and terminalizes the Run, leaving nothing for a watchdog to close. A pass that has not answered yet is the ordinary shape in production and the impossible one for a fixture, and the watchdog reads exactly one thing about a pass - the status its durable run carries. The claim, the `markExecuting` write and the dispatch ordering are all the real ones in those cases; only what `start()` starts is the test's - and, for the abandoned case alone, the `markExecuting` port, which is replaced by one that rejects before its durable write lands, so the pass id is never written. The steps compose their own control plane from `process.env` in a module registry the test file does not share, which is the builder-dependence this package exists for, exercised rather than assumed.

The files beside it that need neither a World nor a database say so by not asking for one. `pass.test.ts` drives the composition seam over loaders of its own - an absent driver, a broken one, and one whose *own* dependency is missing, which is the failure a bare error-code check would misreport as self-hosted. `dispatch.test.ts` measures the wiring `dispatchHostedPass` is: `not_composed` where nothing is composed, and the control plane's own statements behind the ports it hands the ordering. `lifecycle.test.ts` enumerates the watchdog's observation mapping, which is the one part of the liveness branch that can be enumerated exhaustively. `ingress.test.ts` enumerates `concludeDispatch`, for the reason above: the branch it decides runs only on an interleaving the shipped ordering wins in one round trip, so the decision is separated from the step rather than reached through it.
