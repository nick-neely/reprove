...
The window is a fact about the **Run row**, not about a particular way of crashing: the claim landed, `start()` returned, and no pass id was ever written. So a `markExecuting` port that never returns reaches it exactly. `dispatch.test.ts` here does that over in-memory ports: the claim and the start happened, the dispatch rejects, and the recording port never reached an implementation that could write - which is that row stated in ports; `spine.test.ts` in `@reprove/control-plane-workflow` does it against the real control plane and a real durable pass, and reads back the `claimed`/null-pass row this ordering leaves.
