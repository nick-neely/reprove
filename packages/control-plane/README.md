...
**`bootstrap` runs before `migrate`, and the two are not interchangeable.** Every generated migration carries `CREATE POLICY ... TO "reprove_runtime"`, which fails outright if the role does not exist yet, so `migrate` refuses rather than failing halfway through. `bootstrap` provisions the restricted role and the reach it has before any table exists - `CONNECT`, `USAGE` on `public`, read on the migration ledger, and the revocations that keep it from creating a relation of its own - and creates no table.
