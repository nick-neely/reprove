...
const WORKSPACES = {
  ...
  "packages/control-plane-workflow": {
    ...
    // workflows under; it is declared as a devDependency and imported from no
    // shipped file, and it is named here because the shared-tooling exemption
    // covers only the test runner itself.
    external: ["workflow", "@workflow/vitest"],
    forbidden: [
      "@reprove/worker-core",
      "@reprove/adapters",
      "@reprove/sandbox-container",
...
const UNSCANNED_DIRECTORIES = new Set([
  ...
  ".next",
  ".turbo",
  // Generated by the Workflow build and the Workflow test builder: routes
  // `withWorkflow` writes into the app tree, and the bundles `@workflow/vitest`
  // emits. Both are build output that imports the SDK's own internals, and
  // neither is a boundary anybody authored.
  ".well-known",
  ".workflow-vitest",
  ".workflow-data",
]);
